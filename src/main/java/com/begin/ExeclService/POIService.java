...
//        sb.setId(UUID.randomUUID().node());
        ...
        sb.setFwdx(getStringValue(sheet.getRow(3 + i).getCell(1)) + getStringValue(sheet.getRow(3).getCell(3)));
        sb.setFrzt(getStringValue(sheet.getRow(4 + i).getCell(1)));
...
//        sb.setSltj(getStringValue(sheet.getRow(22+i).getCell(0)));
    ...
    public List<Map<String, String>> readExcel2(File file) {
        ...
        for (int i = 1; i <= shuRowNum; i += (mergedRegion + 1)) {
            Row row1 = sheet.getRow(i);
            mergedRegion = isMergedRegion(sheet, i);
            if (row1 == null || isRowEmpty(row1)) continue;
            String hb = null;
            for (int j = i; j <= mergedRegion + i; j++) {
                Row row2 = sheet.getRow(j);
                Map<String, String> mapString = new HashMap<>();
                count++;
                for (int k = (index + 0); k < lastCellNum; k++) {
                    if (row.getCell(k) == null) continue;
                    String stringKey1 = row.getCell(k).getStringCellValue();
                    ...
                    if (cell1 != null) {
                        CellType cellTypeEnum = cell1.getCellTypeEnum();
                        stringValues = getStringValue(row2.getCell(k));
                        StringUtils.trim(stringValues);
                        if (k == 0 && stringValues != "") hb = stringValues;
                        if (k == 0 && stringValues == "") stringValues = hb;
                    } else {
                        stringValues = "null";
                    }
...
//        Iterator<Cell> iterator = row.iterator();
    ...
    public String getCellValue(Sheet sheet, int row, int column) {
        ...
        value = getCellValue(sheet.getRow(mergedRegion.getStartRow()).getCell(mergedRegion.getStartCol()));
        return value;
    }
